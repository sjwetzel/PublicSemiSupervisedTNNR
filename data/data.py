...
#def bostonHousing(path='./'):   ##################### this version seems to interfere with the random number generator
#    from tensorflow.keras.datasets import boston_housing
#    return boston_housing.load_data(test_split=0.)[0]


#a=bostonHousing2()
#np.savetxt('BostonHousing.csv', np.concatenate((a[0],a[1].reshape(506,1)),axis=1), delimiter=',')
# def bostonHousing(path='./'):
#     d = np.loadtxt(path + 'BostonHousing.csv', delimiter=',')
#     return (d[:, :-1], d[:, -1])
...
def splitData(all_x, all_y, val_pct=0.05, test_pct=0.05,rand=True):
    ...
    if rand==False:
        all_indices=np.array(list(range(N_total)))
...
def testFunction(Number_of_datapoints):
    
    def f(x):
        return x[0]**3+x[0]**2-x[0]-1+np.sin(x[1])+x[0]*x[1]
    np.random.seed(0)
    x_full=np.random.sample([Number_of_datapoints,2])*2-1
    y_full=(np.array([f(x) for x in x_full])).flatten()

    return (x_full, y_full)

def getData(key, path='./data/', n_points=1000):
    keymap = {'bostonHousing': bostonHousing(path),
          ...
          'wine': wine(path),
          'Biocon': BioCon(path),
          'testFunction': testFunction(n_points)
         }
    return keymap[key]
